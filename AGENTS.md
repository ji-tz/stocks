...
最后更新：2026-06-16（v3 — 新增开发工作流：ARCH 拆解任务 → 各角色分步实现 → LEAD 审查 → QA 验收 → 合并）

---

## 九、开发工作流（所有改动必须遵守）

> **每一条 Issue，无论大小，都必须走完这个完整流程。** 没有"小改动快车道"，没有"直接改"。每一条 Issue 都是：ARCH 拆解 → 角色分任务 → 实现 → 推送 → CI → LEAD 审查 → QA 验证 → 合并。
...
用户提需求
  │
  ▼
步骤1 建 Issue ──── Hermes 在 GitHub 创建 Issue，写清需求描述
  │
  ▼
步骤2 ARCH 拆解 ─── 架构师（Hermes）评论发布拆解方案：
  │                   - 识别涉及哪些角色（EXCH / TRADER / STRAT / GUI / ITEST / GTEST）
  │                   - 每个角色分配具体任务（涉及哪些文件、做什么改动）
  │                   - 标注任务大小（小/中/大）
  │                   - 打角色标签（exchange / trader / strategy / gui / test / ci）
  │
  ▼
步骤3 建分支 + PR ── 从 main 开分支 feat/issue-<num>-<desc>，创建 PR，贴拆解方案 + checkbox
  │
  ▼
步骤4 各角色实现 ── 按角色标签依次（或链式）执行：
  │   Role A（如 EXCH）→ 改代码 → 自测 → commit → push → Issue 评论完成 → 勾 checkbox
  │   Role B（如 GUI） → 改代码 → 自测 → commit → push → Issue 评论完成 → 勾 checkbox
  │   如涉及测试 → ITEST/GTEST 负责对应测试
  │
  ▼
步骤5 CI 自动运行 ─── GitHub Actions 运行 4 个 required checks
  │
  ▼
步骤6 LEAD 审查 ──── 研发主管审查 PR diff：
  │                   - 检查文件所有权合规
  │                   - 检查代码质量
  │                   - 仲裁 CI 失败（dev-bug / test-needs-update / flaky）
  │                   - 在 PR 评论发布审查结果
  │
  ▼
步骤7 QA 验收 ────── 验收测试：
  ...
  │
  ▼
步骤8 合并 ───────── squash-merge PR 到 main
  │                   - 确认 CI 全部通过
  │                   - 关闭 Issue
  │
  ▼
步骤9 部署 ───────── stock-nightly cron 每天 18:00 自动拉 main 重启
```
...
| 角色 | 负责的目录 | 改动时必须 |
|------|-----------|-----------|
| **ARCH** | — | 拆解需求，分配任务到各角色，发布拆解方案评论 |
| **EXCH** | `exchange/` | 数据源、交易所逻辑、符号处理 |
| **TRADER** | `trader/` | 回测引擎、交易逻辑、时间推进 |
| **STRAT** | `strategy/` | 策略算法、信号生成 |
| **GUI** | `gui/` | Web 前端模板、Flask 路由、JS 交互 |
| **ITEST** | `tests/unit/`, `tests/integration/` | 单元/集成测试 |
| **GTEST** | `tests/guitests/` | GUI 端到端测试 |
| **LEAD** | — | Review PR diff，仲裁 CI 失败 |
| **QA** | — | 验收测试，确认满足需求，合并 PR |

**每角色的实现流程：**
1. 拉取最新的 PR 分支（`git fetch origin feat/issue-N-desc && git checkout feat/issue-N-desc`）
2. 仅修改自己角色目录下的文件
3. 运行相关测试确认无回归
4. commit + push
5. 在 Issue 评论报告完成，更新 PR checkbox
...
- ❌ **不在 stock-nightly/ 目录直接改代码**
- ❌ **不等 CI 就合并**
- ❌ **不在 CI 失败时继续推进**
- ❌ **不跳过任何一个角色（ARCH → 实现 → LEAD → QA → 合并）**
