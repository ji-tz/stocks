...
## 多Agent协作
- 使用subagent来进行不同模块、功能的开发和测试。
- 由主Agent进行任务的分解，讲任务分配给不同的subAgent来完成。每个subAgent负责一个具体的功能或模块的开发和测试。
- 建立subAgent时，由Agent告诉它它的任务和目标，并提供必要的上下文信息。subAgent完成任务后，返回结果到Agent处汇总，清理subAgent和它的上下文。以避免subAgent对主Agent的上下文占用。
